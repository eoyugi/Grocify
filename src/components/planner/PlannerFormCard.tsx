...
import { Compass, Hash, Plus, ShoppingBag, Sprout, Zap } from 'lucide-react-native';
  ...
  return (
    <View className="rounded-3xl border border-border bg-card p-4">
      ...
      <View className="mt-2 flex-row items-center rounded-2xl border border-border bg-muted px-4 py-3">
        <ShoppingBag size={13} color="#5b7567" />
        <TextInput
          value={name}
      ...
      <View className="mt-2 flex-row flex-wrap gap-2">
        {categories.map((option) => {
          ...
          return (
            ...
            >
              <Text className="text-sm">{categoryIcons[option]}</Text>
              <Text
      ...
      <View className="mt-2 flex-row gap-2">
        {priorities.map((option) => {
          ...
          const Icon = priorityIconMap[option];
          return (
            <Pressable
              key={option}
            ...
            >
              <Icon size={12} color={active ? '#ffffff' : '#486856'} />
              <Text
      ...
      >
        <Plus size={14} color={canCreate ? '#ffffff' : '#7a9386'} />
        <Text
